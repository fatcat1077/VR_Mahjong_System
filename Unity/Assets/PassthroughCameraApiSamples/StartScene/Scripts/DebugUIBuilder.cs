...
namespace PassthroughCameraSamples.StartScene
{
    ...
    public class DebugUIBuilder : MonoBehaviour
    {
        ...
        public void ShowAtCurrentHeadPose()
        {
            ShowInternal(true);
        }

        private void ShowInternal(bool useCurrentHeadPose)
        {
            Relayout();
            gameObject.SetActive(true);
            var reference = m_rig != null && m_rig.centerEyeAnchor != null
                ? m_rig.centerEyeAnchor
                : (m_rig != null ? m_rig.transform : null);

            if (reference != null)
            {
                var newEulerRot = (useCurrentHeadPose ? reference.rotation : m_rig.transform.rotation).eulerAngles;
                newEulerRot.x = 0.0f;
                newEulerRot.z = 0.0f;
                var yawRotation = Quaternion.Euler(newEulerRot);
                transform.rotation = yawRotation;
                transform.position = useCurrentHeadPose
                    ? reference.position + yawRotation * m_menuOffset
                    : m_rig.transform.TransformPoint(m_menuOffset);
            }
        ...
        public void Clear()
        {
            if (m_insertedElements == null || m_insertPositions == null)
                return;

            for (var panelIdx = 0; panelIdx < m_insertedElements.Length; ++panelIdx)
            {
                var elems = m_insertedElements[panelIdx];
                for (var elemIdx = elems.Count - 1; elemIdx >= 0; --elemIdx)
                {
                ...
                if (panelIdx < m_insertPositions.Length)
                {
                    m_insertPositions[panelIdx].x = MarginH;
                    m_insertPositions[panelIdx].y = -MarginV;
                }

                if (panelIdx < m_targetContentPanels.Length && m_targetContentPanels[panelIdx] != null)
                {
                    m_targetContentPanels[panelIdx].gameObject.SetActive(false);
                }
            }
        }
